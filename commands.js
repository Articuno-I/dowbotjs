...
exports.commands = {
    ...
    mgc: 'makeGroupChat',
    makeGroupChat: function(arg, by, room) {
        if (room.charAt(0) !== ',') return;
        if (!(toId(by) in aether_ranks)) {
            send(room + "|DoWbot.js is a bot made for the monotype room by DoW. Commands: https://pastebin.com/BxPgGuhh");
            return;
        }
        //check room doesn't already exist
        send('monotype|/makegroupchat aether, private');
        setTimeout(function() {
            send('groupchat-dowbotjs-aether|/modjoin +');
            send("groupchat-dowbotjs-aether|/roomintro <center><h2>Team Aether's Headquarters</h2></center><p>Rules:</p><p>1. Play to win (within the rules)</p><p>2. Don't be a dick</p><center><p><a href='dowiangnulinux.co.uk:8000/resources'>Resources</a>, <a href='dowiangnulinux.co.uk:8000/guidelines'>Room Guidelines</a></p><h3>Don't invite random people to the room!</h3></center>");
            send("groupchat-dowbotjs-aether|/staffintro <p><a href='dowiangnulinux.co.uk:8000/structure'>Guidelines for various ranks</a>, <a href='dowiangnulinux.co.uk:8000/'>Inviting people to the room</a></p><h3>Remember not to let invite spam happen, mkay</h3>");
        }, 1000);
   },

    inviteUser: function(arg, by, room) {
        if (room.charAt(0) === ',') {
            send(room + "|DoWbot.js is a bot made for the monotype room by DoW. Commands: https://pastebin.com/BxPgGuhh");
            return;
        }
        if (room !== 'groupchat-dowbotjs-aether') return;
        if (!(toId(by) in aether_ranks)) {
            send(room + '|Please log in to an account I recognise.');
            return;
        }
        switch (aether_ranks[toId(by)]) {
            case '@':
                aether_ranks[toId(arg)] = '+';
                send(room + '|/roomvoice ' + toId(arg));
                send(room + '|/invite ' + toId(arg));
                rewriteranks();
                return;
            case '%':
            case '+':
                send(room + '|This can only be done by admins or higher.');
        }
    },

    joinaether: function(arg, by, room) {
        if (room.charAt(0) !== ',') return;
        if (!(toId(by) in aether_ranks)) {
            send(room + "|DoWbot.js is a bot made for the monotype room by DoW. Commands: https://pastebin.com/BxPgGuhh");
            return;
        ...
        switch (aether_ranks[toId(by)]) {
            case '@':
                comm = '/roommod'; break;
            case '%':
                comm = '/roomdriver'; break;
            case '+':
                comm = '/roomvoice'; break;
        }
        send('groupchat-dowbotjs-aether|' + comm + ' ' + toId(by));
        send('groupchat-dowbotjs-aether|/invite ' + toId(by));
    },

    promoteuser: function(arg, by, room) {
        if (room.charAt(0) === ',') {
            send(room + "|DoWbot.js is a bot made for the monotype room by DoW. Commands: https://pastebin.com/BxPgGuhh");
            return;
        }
        if (room !== 'groupchat-dowbotjs-aether') return;
        if (aether_ranks[toId(by)] !== '@') return;
        if (aether_ranks[toId(arg)] === '+') {
            aether_ranks[toId(arg)] = '%';
            send(room + '|/roomdriver ' + toId(arg));
        } else if (aether_ranks[toId(arg)] === '%') {
            aether_ranks[toId(arg)] = '@';
            send(room + '|/roommod ' + toId(arg));
        } else { return; }
        rewriteranks();
    },
