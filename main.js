...
global.toId = function(text) {
	assert.ok(typeof text === 'string', 'Tried to get Id of ' + typeof text + ': ' + text);
	if (text === '') return text;
...
global.Tours = require('./tours.js').tours;
global.MMC = require('./mmc.js').mmc;
global.OMMMC = require('./omMonthlyChampionship.js').OMmmc;
global.BLT = require('./blt.js').blt;

global.Aliases = require('./aliases.js').aliases;
//global.Metagames = require('./metagames.js').metagames;
global.metagames = require('./new_metagames.js').metagames;
//using lower case so it's clear where there's a problem due to new code
global.TeamValidator = require('./newValidator.js').teamValidator;
global.HTML = require('./htmlbox.js').html; 

// global.ModerateChat = require('./chatModeration.js').moderate;
// global.FindUser = require('./seen.js').seen;
...
var connect = function(retry) {
	...
	ws.on('connectFailed', function(err) {
		...
		Tours.tourRunning = false; //for when server restarts and tour is ended w/o receiving any info on it.
	...
	ws.on('connect', function(con) {
		...
		global.connect_time = Math.floor(Date.now()/1000); //shouldn't need more accurate than seconds
